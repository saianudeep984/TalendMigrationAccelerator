...
def render_upgrade_advisor_dashboard(jobs=None, data=None):
    ...
    section_header("Breaking Changes"); st.dataframe(data["breaking_changes"]["findings"], use_container_width=True)
    section_header("Deprecated Components"); st.dataframe(data["deprecated_components"]["findings"], use_container_width=True)
